...
private:
    ...
    int QuadracticProbing(int key, int value) {
        return (int) (((double) key + ((value + pow(value, 2)) / 2)) % (double) capacity);
    }
...
public:
    ...
    void insert(int value) {
        ...
        int HashIndex = 0;
        int ProbeFactor = QuadracticProbing(HashIndex, key);
        ...
        while (HashTable[ProbeFactor] != -1 && HashTable[ProbeFactor] != -2) {
            ...
            HashIndex++;
            ProbeFactor = QuadracticProbing(key, HashIndex);
        }
